...
val seedRangeList = seeds.split(" ").map { it.toLong() }

val seedRanges = mutableListOf<LongRange>()

for (i in 0 until seedRangeList.size - 1 step 2) {
    seedRanges.add(seedRangeList[i] until seedRangeList[i] + seedRangeList[i + 1]) 
}
