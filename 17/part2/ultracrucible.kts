...
while (!queue.isEmpty()) {
    ...
    if (currLoss < memo[coord]!![step.direction][step.stepsTaken - 1]!!) {
        ...
        if (coord == (heatMap.size - 1) to (heatMap[0].length - 1) && steps >= 4) {
            lowestHeatLoss = minOf(lowestHeatLoss, currLoss)
            break
        }
        if (steps >= 4) queue.offer(Step(nextL, l, 1, currLoss))
        if (steps >= 4) queue.offer(Step(nextR, r, 1, currLoss))
        if (steps < 10) queue.offer(Step(nextS, dir, steps + 1, currLoss))
    } 
}
